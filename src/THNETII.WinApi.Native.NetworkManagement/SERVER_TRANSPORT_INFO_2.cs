...
{
    /// <summary>
    /// The <see cref="SERVER_TRANSPORT_INFO_2"/> structure contains information about the specified transport protocol, including the transport name and address. This information level is valid only for the <see cref="NetServerTransportAddEx"/> function.
    /// </summary>
    /// <remarks>
    /// <para>The <see cref="SERVER_TRANSPORT_INFO_2"/> structure is used by the <see cref="NetServerTransportAddEx"/> function to bind the specified server to the transport protocol.</para>
    /// <para>An example of the use of the <see cref="SVTI2_REMAP_PIPE_NAMES"/> value follows. Call the <see cref="NetServerTransportAddEx"/> function to add a transport to the server, specifying the address of "MyServer" in the <see cref="svti2_transportaddress"/> member, and <see cref="SVTI2_REMAP_PIPE_NAMES"/> in the <see cref="svti2_flags"/> member. When a client attempts to open "Pipe" on "\\MyServer" the client will actually open $$MyServer\Pipe instead.</para>
    ...
    /// <para>A scoped endpoint is created by calling the <see cref="NetServerTransportAddEx"/> function with the <var>level</var> parameter set to <c>2</c> and the <var>bufptr</var> parameter pointed to a <see cref="SERVER_TRANSPORT_INFO_2"/> structure with the <see cref="SVTI2_SCOPED_NAME"/> bit value set in <see cref="svti2_flags"/> member. A scoped endpoint is also created by calling the <see cref="NetServerTransportAddEx"/> function with the <var>level</var> parameter set to <c>3</c> and the <var>bufptr</var> parameter pointed to a <see cref="SERVER_TRANSPORT_INFO_3"/> structure with the <see cref="SVTI2_SCOPED_NAME"/> bit value set in <see cref="SERVER_TRANSPORT_INFO_3.svti3_flags"/> member. </para>
    ...
    public class SERVER_TRANSPORT_INFO_2
    {
        /// <summary>
        /// The number of clients connected to the server that are using the transport protocol specified by the <see cref="svti2_transportname"/> member.
        /// </summary>
        public int svti2_numberofvcs;
        /// <summary>
        ...
        /// <summary>
        /// <para>A pointer to a variable that contains the address the server is using on the transport device specified by the <see cref="svti2_transportname"/> member.</para>
        ...
        /// </summary>
        public ByteArrayAnySafeHandle svti2_transportaddress;
        /// <summary>
        /// The length, in bytes, of the <see cref="svti2_transportaddress"/> member. For NetBIOS names, the value of this member is 16 (decimal).
        /// </summary>
        public int svti2_transportaddresslength;
        /// <summary>
        /// Gets the address the server is using on the transport device specified by the <see cref="svti2_transportname"/> member.
        ...
        /// </summary>
        /// <returns>A managed byte array containing exactly <see cref="svti2_transportaddresslength"/> items, or <c>null</c> if <see cref="svti2_transportaddress"/> is <c>null</c>.</returns>
        public byte[] GetTransportAddress() => ByteArraySafeHandle.ReadValue(svti2_transportaddress, svti2_transportaddresslength);
        /// <summary>
        ...
        public SVTI2_FLAGS svti2_flags;
    }
}
