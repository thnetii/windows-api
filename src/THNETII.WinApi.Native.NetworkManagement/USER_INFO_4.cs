...
{
    /// <summary>
    /// The <see cref="USER_INFO_4"/> structure contains information about a user account, including the account name, password data, privilege level, the path to the user's home directory, relative identifiers (RIDs), and other user-related network statistics.
    /// </summary>
    /// <remarks>
    /// <para>The <see cref="USER_INFO_4"/> structure can be used with the <see cref="NetUserAdd"/>, <see cref="NetUserEnum"/> , <see cref="NetUserSetInfo"/>, and <see cref="NetUserGetInfo"/> functions.</para>
    ...
    /// <para>Original MSDN documentation page: <a href="https://msdn.microsoft.com/en-us/library/aa371339.aspx">USER_INFO_4 structure</a></para>
    /// </remarks>
    ...
    public class USER_INFO_4
    {
        ...
        /// <summary>
        /// A string that specifies the password of the user indicated by the <see cref="usri4_name"/> member. The length cannot exceed <see cref="PWLEN"/> bytes. The <see cref="NetUserEnum"/> and <see cref="NetUserGetInfo"/> functions return <c>null</c> to maintain password security. 
        /// <para>By convention, the length of passwords is limited to <see cref="LM20_PWLEN"/> characters.</para>
        /// </summary>
        [MarshalAs(UnmanagedType.LPWStr)]
        public string usri4_password;
        /// <summary>
        /// The number of seconds that have elapsed since the <see cref="usri4_password"/> member was last changed. The <see cref="NetUserAdd"/> and <see cref="NetUserSetInfo"/> functions ignore this member.
        /// </summary>
        public int usri4_password_age;
        /// <summary>
        /// The level of privilege assigned to the <see cref="usri4_name"/> member. When you call the <see cref="NetUserAdd"/> function, this member must be <see cref="USER_PRIV_USER"/>. When you call the <see cref="NetUserSetInfo"/> function, this member must be the value returned by the <see cref="NetUserGetInfo"/> function or the <see cref="NetUserEnum"/> function. For more information about user and group account rights, see <a href="https://msdn.microsoft.com/en-us/library/aa379306.aspx">Privileges</a>. 
        /// </summary>
        [MarshalAs(UnmanagedType.I4)]
        public USER_PRIVILEGE_LEVEL usri4_priv;
        /// <summary>
        /// A string specifying the path of the home directory for the user specified in the <see cref="usri4_name"/> member. The string can be <c>null</c>.
        /// </summary>
        [MarshalAs(UnmanagedType.LPWStr)]
        ...
        /// </summary>
        [MarshalAs(UnmanagedType.I4)]
        public ACCOUNT_OPERATOR_FLAGS usri4_auth_flags;
        /// <summary>
        ...
        /// </summary>
        /// <seealso cref="usri4_last_logon"/>
        public DateTime LastLogon => epoch.AddSeconds((uint)usri4_last_logon).ToLocalTime();
        /// <summary>
        ...
        /// </summary>
        /// <seealso cref="usri4_last_logoff"/>
        public DateTime LastLogoff => (usri4_last_logoff == 0 ? default(DateTime) : epoch.AddSeconds((uint)usri4_last_logoff).ToLocalTime());
        /// <summary>
        ...
        {
            get { return usri4_acct_expires == TIMEQ_FOREVER ? DateTime.MaxValue : epoch.AddSeconds((uint)usri4_acct_expires).ToLocalTime(); }
            set
            {
                if (value == default(DateTime))
                    usri4_acct_expires = 0;
                else if (value == DateTime.MaxValue)
                    usri4_acct_expires = TIMEQ_FOREVER;
                else
                    usri4_acct_expires = (int)((uint)(value - epoch).TotalSeconds);
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// <para>The number of equal-length time units into which the week is divided. This value is required to compute the length of the bit string in the <see cref="usri4_logon_hours"/> member. </para>
        ...
        /// </summary>
        public ByteArrayAnySafeHandle usri4_logon_hours;
        /// <summary>
        /// Gets the marshaled bit array containing the logon hours during which the user can log on.
        /// </summary>
        /// <returns>A bit array containing exactly <see cref="usri4_units_per_week"/> bits, or zero bits if <see cref="usri4_logon_hours"/> is <c>null</c>.</returns>
        /// <seealso cref="usri4_logon_hours"/>
        /// <seealso cref="usri4_units_per_week"/>
        public BitArray GetLogonHoursBitArray() => new BitArray(ByteArraySafeHandle.ReadValue(usri4_logon_hours, usri4_units_per_week / 8) ?? new byte[0]);
        /// <summary>
        ...
        /// </summary>
        public IntPtr usri4_user_id;
        /// <summary>
        /// Gets the security identifier (SID) that uniquely identifies the user.
        /// </summary>
        /// <returns>A <see cref="SecurityIdentifier"/> instance for the user, or <c>null</c> if <see cref="usri4_user_id"/> is <see cref="IntPtr.Zero"/>.</returns>
        public SecurityIdentifier GetUserId() => usri4_user_id == IntPtr.Zero ? null : new SecurityIdentifier(usri4_user_id);
        /// <summary>
        ...
        public bool usri4_password_expired;
    }
}
