...
{
    /// <summary>
    /// All Win32 error codes MUST be in the range <c>0x0000</c> to <c>0xFFFF</c>, although Win32 error codes can be used both in 16-bit fields (such as within the <see cref="HRESULT"/> type) as well as 32-bit fields. Most values also have a default message defined, which can be used to map the value to a human-readable text message; when this is done, the Win32 error code is also known as a message identifier.
    /// </summary>
    /// <remarks>
    /// <note>In the following field descriptions, a percentage sign followed by one or more alphanumeric characters (for example, "%1" or "%hs") indicates a variable that will be replaced by text at the time the value is returned.</note>
    /// <para>Original MSDN documentation page: <a href="https://msdn.microsoft.com/en-us/library/cc231199.aspx">Win32 Error Codes</a></para>
    /// </remarks>
